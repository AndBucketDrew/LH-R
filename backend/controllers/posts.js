...
const createPost = async (req, res, next) => {
    try {
        ...
        if (!image) { throw new HttpError('Image is required', 400) };
        if (!author) { throw new HttpError('author is required', 400) };
...
const toggleLike = async (req, res, next) => {
    try {
        ...
        await post.save();
        res.status(200).json({ liked: !alreadyLiked, likeCount: post.likes.length });
    } catch (error) {
        return next(new HttpError(error, error.errorCode || 500));
    }
};
